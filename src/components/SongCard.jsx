  ...
  return (
    <div className="flex flex-col w-[320px] p-4 bg-white/5 bg-opacity-80 backdrop-blur-sm animate-slideup rounded-lg cursor-pointer">
      <div className="relative w-full h-72 group">
        ...
        <img alt="song_img" src={song?.album?.images[0]?.url} className="w-full h-full rounded-lg object-cover" />
      </div>

      <div className="mt-4 flex flex-col">
        <p className="font-semibold text-lg text-white truncate">
          <Link to={`/songs/${song?.id}`}>
            <p className="font-bold text-lg text-[#e34814] truncate mt-4">{song?.name}</p>
          </Link>
        </p>
        <p className="text-sm truncate text-gray-300 mt-1">
          <Link to={song?.artists?.length ? `/artists/${song.artists[0]?.id}` : '/top-artists'}>
            {song?.artists?.[0]?.name}
          </Link>
        </p>
      </div>
